...
export default function ConnectWalletButton() {
  ...
  const { address, isAuthenticated, isNetwork, chainId } = account;
  ...
  return (
    <>
      {!isAuthenticated && !address ? (
        ...
        <>
          {!isNetwork ? (
            <button className="flex flex-row items-center gap-[10px] rounded-lg bg-alt-500 px-[29px] py-[15px] font-rob text-[14px]/[19px] text-white">
              <ExclamationTriangleIcon className="h-5 w-5 text-white" /> Oops! Network not supported.
            </button>
          ) : (
